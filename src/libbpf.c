    ...
    if (!initialized) {
        ...
        if (verbosity) {
            if (strcasecmp(verbosity, "warn") == 0)
                ...
                fprintf(stderr,
                        "libbpf: unrecognized '%s' envvar value: '%s', should be one of 'warn', "
                        "'debug', or 'info'.\n",
                        env_var,
                        verbosity);
        }
...
static int bpf_object__add_programs(struct bpf_object *obj,
                                    Elf_Data *sec_data,
                                    const char *sec_name,
                                    int sec_idx)
{
    ...
    for (i = 0; i < nr_syms; i++) {
        ...
        if (sec_off + prog_sz > sec_sz) {
            pr_warn(
                "sec '%s': program at offset %zu crosses section boundary\n", sec_name, sec_off);
            return -LIBBPF_ERRNO__FORMAT;
        }
        ...
        pr_debug("sec '%s': found program '%s' at insn offset %zu (%zu bytes), code size %zu insns "
                 "(%zu bytes)\n",
                 sec_name,
                 name,
                 sec_off / BPF_INSN_SZ,
        ...
        err = bpf_object__init_prog(
            obj, prog, name, sec_idx, sec_name, sec_off, data + sec_off, prog_sz);
        if (err)
            return err;
...
static int find_ksym_btf_id(struct bpf_object *obj,
...
static int find_btf_by_prefix_kind(const struct btf *btf,
                                   const char *prefix,
                                   const char *name,
                                   __u32 kind);
                                      ...
                                      const struct btf_member **data_member)
{
    ...
    kern_type_id = find_ksym_btf_id(obj, tname, BTF_KIND_STRUCT, &btf, mod_btf);
    if (kern_type_id < 0) {
...
static bool is_valid_st_ops_program(struct bpf_object *obj, const struct bpf_program *prog)
{
    int i;
...
static int bpf_map__init_kern_struct_ops(struct bpf_map *map)
{
    ...
    err = find_struct_ops_kern_types(obj,
                                     tname,
                                     &mod_btf,
                                     &kern_type,
                                     &kern_type_id,
    ...
    for (i = 0; i < btf_vlen(type); i++, member++) {
        ...
        if (btf_is_ptr(mtype)) {
            ...
            if (prog->attach_btf_id != kern_type_id) {
                pr_warn("struct_ops init_kern %s func ptr %s: invalid reuse of prog %s in sec %s "
                        "with type %u: attach_btf_id %u != kern_type_id %u\n",
                        map->name,
                        mname,
                        prog->name,
                        prog->sec_name,
            ...
            if (prog->expected_attach_type != kern_member_idx) {
                pr_warn("struct_ops init_kern %s func ptr %s: invalid reuse of prog %s in sec %s "
                        "with type %u: expected_attach_type %u != kern_member_idx %u\n",
                        map->name,
                        mname,
                        prog->name,
                        prog->sec_name,
            ...
            pr_debug("struct_ops init_kern %s: func ptr %s is set to prog %s from data(+%u) to "
                     "kern_data(+%u)\n",
                     map->name,
                     mname,
                     prog->name,
...
static int init_struct_ops_maps(struct bpf_object *obj,
                                const char *sec_name,
                                int shndx,
                                Elf_Data *data)
{
    ...
    for (i = 0; i < btf_vlen(datasec); i++, vsi++) {
        ...
        if (vsi->offset + type->size > data->d_size) {
            pr_warn(
                "struct_ops init: var %s is beyond the end of DATASEC %s\n", var_name, sec_name);
            return -EINVAL;
        }
...
static int bpf_object__elf_init(struct bpf_object *obj)
{
    ...
    if (!elf_rawdata(elf_getscn(elf, obj->efile.shstrndx), NULL)) {
        pr_warn(
            "elf: failed to get section names strings from %s: %s\n", obj->path, elf_errmsg(-1));
        err = -LIBBPF_ERRNO__FORMAT;
        goto errout;
    }
...
static struct bpf_map *bpf_object__add_map(struct bpf_object *obj)
{
    ...
    err = libbpf_ensure_mem(
        (void **) &obj->maps, &obj->maps_cap, sizeof(*obj->maps), obj->nr_maps + 1);
    if (err)
        return ERR_PTR(err);
...
static int bpf_object__init_global_data_maps(struct bpf_object *obj)
{
    ...
    for (sec_idx = 1; sec_idx < obj->efile.sec_cnt; sec_idx++) {
        ...
        switch (sec_desc->sec_type) {
            case SEC_DATA:
                sec_name = elf_sec_name(obj, elf_sec_by_idx(obj, sec_idx));
                err = bpf_object__init_internal_map(obj,
                                                    LIBBPF_MAP_DATA,
                                                    sec_name,
                                                    sec_idx,
                                                    sec_desc->data->d_buf,
            ...
            case SEC_BSS:
                sec_name = elf_sec_name(obj, elf_sec_by_idx(obj, sec_idx));
                err = bpf_object__init_internal_map(
                    obj, LIBBPF_MAP_BSS, sec_name, sec_idx, NULL, sec_desc->data->d_size);
                break;
            default:
                /* skip */
                break;
...
static int bpf_object__init_kconfig_map(struct bpf_object *obj)
{
    ...
    err = bpf_object__init_internal_map(
        obj, LIBBPF_MAP_KCONFIG, ".kconfig", obj->efile.symbols_shndx, NULL, map_sz);
    if (err)
        return err;
                              ...
                              __u32 *res)
{
    ...
    if (!btf_is_array(arr_t)) {
        pr_warn(
            "map '%s': attr '%s': expected ARRAY, got %s.\n", map_name, name, btf_kind_str(arr_t));
        return false;
    }
...
static bool get_map_field_long(const char *map_name,
                               const struct btf *btf,
                               const struct btf_member *m,
                               __u64 *res)
{
...
int parse_btf_map_def(const char *map_name,
                      struct btf *btf,
                      const struct btf_type *def_t,
                      bool strict,
...
static int init_arena_map_data(struct bpf_object *obj,
                               struct bpf_map *map,
                               const char *sec_name,
                               int sec_idx,
                               void *data,
                               size_t data_sz)
{
    ...
    if (roundup(data_sz, page_sz) > mmap_sz) {
        pr_warn("elf: sec '%s': declared ARENA map size (%zu) is too small to hold global __arena "
                "variables of size %zu\n",
                sec_name,
                mmap_sz,
                data_sz);
        return -E2BIG;
                                          ...
                                          const char *pin_root_path)
{
    ...
    vlen = btf_vlen(sec);
    for (i = 0; i < vlen; i++) {
        err = bpf_object__init_user_btf_map(
            obj, sec, i, obj->efile.btf_maps_shndx, data, strict, pin_root_path);
        if (err)
            return err;
    }
    ...
    if (obj->efile.arena_data && !obj->arena_map) {
        pr_warn("elf: sec '%s': to use global __arena variables the ARENA map should be explicitly "
                "declared in SEC(\".maps\")\n",
                ARENA_SEC);
        return -ENOENT;
    }
...
{
    return obj->efile.btf_maps_shndx >= 0 || obj->efile.has_st_ops || obj->nr_extern > 0;
}
...
static int bpf_object__init_btf(struct bpf_object *obj, Elf_Data *btf_data, Elf_Data *btf_ext_data)
{
    ...
    if (btf_ext_data) {
        ...
        if (err) {
            pr_warn(
                "Error loading ELF section %s: %d. Ignored and continue.\n", BTF_EXT_ELF_SEC, err);
            obj->btf_ext = NULL;
            goto out;
        }
...
static int bpf_object__sanitize_and_load_btf(struct bpf_object *obj)
{
    ...
    if (obj->gen_loader) {
        ...
        err = btf_load_into_kernel(
            kern_btf, obj->log_buf, obj->log_size, obj->log_level ? 1 : 0, obj->token_fd);
    }
    if (sanitize) {
        if (!err) {
...
static int bpf_object__elf_collect(struct bpf_object *obj)
{
    ...
    while ((scn = elf_nextscn(elf, scn)) != NULL) {
        ...
        if (strcmp(name, "license") == 0) {
            ...
            pr_warn("elf: legacy map definitions in 'maps' section are not supported by libbpf "
                    "v1.0+\n");
            return -ENOTSUP;
            ...
            if (!section_have_execinstr(obj, targ_sec_idx) && strcmp(name, ".rel" STRUCT_OPS_SEC) &&
                strcmp(name, ".rel" STRUCT_OPS_LINK_SEC) && strcmp(name, ".rel?" STRUCT_OPS_SEC) &&
                                     ...
                                     const Elf64_Rel *rel)
{
    ...
    if (!shdr_idx || shdr_idx >= SHN_LORESERVE) {
        pr_warn("prog '%s': invalid relo against '%s' in special section 0x%x; forgot to "
    ...
    if (map_idx >= nr_maps) {
        pr_warn(
            "prog '%s': data relo failed to find map for section '%s'\n", prog->name, sym_sec_name);
        return -LIBBPF_ERRNO__RELOC;
    }
...
static int bpf_object__collect_prog_relos(struct bpf_object *obj, Elf64_Shdr *shdr, Elf_Data *data)
{
    ...
    for (i = 0; i < nrels; i++) {
        ...
        if (!prog) {
            pr_debug("sec '%s': relo #%d: couldn't find program in section '%s' for insn #%u, "
                     "probably overridden weak function, skipping...\n",
                     relo_sec_name,
                     i,
                     sec_name,
                     insn_idx);
...
static int bpf_object_prepare_token(struct bpf_object *obj)
{
    ...
    if (token_fd < 0) {
        if (!mandatory && token_fd == -ENOENT) {
            pr_debug("object '%s': BPF FS at '%s' doesn't have BPF token delegation set up, "
                     "skipping...\n",
                     obj->name,
                     bpffs_path);
            return 0;
...
static int bpf_object__populate_internal_map(struct bpf_object *obj, struct bpf_map *map)
{
    ...
    if (obj->gen_loader) {
        bpf_gen__map_update_elem(
            obj->gen_loader, map - obj->maps, map->mmaped, map->def.value_size);
...
static int bpf_object__create_map(struct bpf_object *obj, struct bpf_map *map, bool is_inner)
{
    ...
    if (obj->gen_loader) {
        bpf_gen__map_create(obj->gen_loader,
                            def->type,
                            map_name,
                            def->key_size,
                            def->value_size,
        ...
        map_fd = bpf_map_create(
            def->type, map_name, def->key_size, def->value_size, def->max_entries, &create_attr);
    }
                                                      ...
                                                      __u32 local_type_id)
{
    ...
    for (i = 0; i < obj->btf_module_cnt; i++) {
        err = bpf_core_add_cands(&local_cand,
                                 local_essent_len,
 ...
 */
int bpf_core_types_are_compat(const struct btf *local_btf,
                              __u32 local_id,
                              const struct btf *targ_btf,
                              __u32 targ_id)
{
...
int bpf_core_types_match(const struct btf *local_btf,
                         __u32 local_id,
                         const struct btf *targ_btf,
                         __u32 targ_id)
{
                                 ...
                                 struct bpf_core_relo_res *targ_res)
{
    ...
    return bpf_core_calc_relo_insn(
        prog_name, relo, relo_idx, local_btf, cands, specs_scratch, targ_res);
}
    ...
    for_each_btf_ext_sec(seg, sec)
    {
        ...
        for_each_btf_ext_rec(seg, sec, i, rec)
        {
            ...
            if (!prog) {
                 ...
                 */
                pr_debug("sec '%s': skipping CO-RE relocation #%d for insn #%d belonging to "
                         "eliminated weak subprogram\n",
                         sec_name,
                         i,
                         insn_idx);
                continue;
...
static void poison_map_ldimm64(struct bpf_program *prog,
                               int relo_idx,
                               int insn_idx,
                               struct bpf_insn *insn,
                               int map_idx,
...
static void poison_kfunc_call(struct bpf_program *prog,
                              int relo_idx,
                              int insn_idx,
                              struct bpf_insn *insn,
                              int ext_idx,
                                         ...
                                         const struct bpf_program *prog)
{
    ...
    err = adjust_prog_btf_ext_info(obj,
                                   prog,
    ...
    if (err) {
        ...
        pr_warn("prog '%s': missing .BTF.ext function info for the main program, skipping all of "
                ".BTF.ext func info.\n",
                prog->name);
    }
...
line_info:
    ...
    if (err) {
        ...
        /* Have problem loading the very first info. Ignore the rest. */
        pr_warn("prog '%s': missing .BTF.ext line info for the main program, skipping all of "
                ".BTF.ext line info.\n",
                prog->name);
    }
    return 0;
...
static struct reloc_desc *find_prog_insn_relo(const struct bpf_program *prog, size_t insn_idx)
{
    if (!prog->nr_reloc)
        return NULL;
    return bsearch(&insn_idx,
                   prog->reloc_desc,
                   prog->nr_reloc,
                   sizeof(*prog->reloc_desc),
...
static int append_subprog_relos(struct bpf_program *main_prog, struct bpf_program *subprog)
{
    ...
    if (subprog->nr_reloc)
        memcpy(
            relos + main_prog->nr_reloc, subprog->reloc_desc, sizeof(*relos) * subprog->nr_reloc);
    ...
    for (i = 0; i < nrels; i++) {
        ...
        if (moff >= map->init_slots_sz) {
            ...
            if (!tmp)
                return -ENOMEM;
            map->init_slots = tmp;
            memset(map->init_slots + map->init_slots_sz,
                   0,
    ...
    switch (prog->type) {
        ...
        case BPF_PROG_TYPE_STRUCT_OPS:
            if (prog->attach_btf_id == 0) {
                pr_warn("prog '%s': SEC(\"struct_ops\") program isn't referenced anywhere, did you "
                        "forget to use it?\n",
                        prog->name);
                return -EINVAL;
            }
...
static void patch_log(
    char *buf, size_t buf_sz, size_t log_sz, char *orig, size_t orig_sz, const char *patch)
{
        ...
        switch (relo->type) {
            case RELO_EXTERN_LD64:
                ...
                kind = btf_is_var(btf__type_by_id(obj->btf, ext->btf_id)) ? BTF_KIND_VAR
                                                                          : BTF_KIND_FUNC;
                bpf_gen__record_extern(obj->gen_loader,
                                       ext->name,
                                       ext->is_weak,
                                       !ext->ksym.type_id,
                                       true,
...
static int bpf_object__load_progs(struct bpf_object *obj, int log_level)
{
    ...
    for (i = 0; i < obj->nr_programs; i++) {
        ...
        err = bpf_object_load_prog(
            obj, prog, prog->insns, prog->insns_cnt, obj->license, obj->kern_version, &prog->fd);
        if (err) {
...
static struct bpf_object *bpf_object_open(const char *path,
                                          const void *obj_buf,
                                          size_t obj_buf_sz,
                                          const char *obj_name,
...
struct bpf_object *bpf_object__open_mem(const void *obj_buf,
                                        size_t obj_buf_sz,
                                        const struct bpf_object_open_opts *opts)
{
    char tmp_name[64];
...
typedef int (*kallsyms_cb_t)(unsigned long long sym_addr,
                             char sym_type,
                             const char *sym_name,
                             void *ctx);
...
static int bpf_object__resolve_ksym_var_btf_id(struct bpf_object *obj, struct extern_desc *ext)
{
    ...
    if (err <= 0) {
        ...
        pr_warn("extern (var ksym) '%s': incompatible types, expected [%d] %s %s, but kernel has "
                "[%d] %s %s\n",
                ext->name,
                local_type_id,
                btf_kind_str(local_type),
...
static int bpf_object__resolve_ksym_func_btf_id(struct bpf_object *obj, struct extern_desc *ext)
{
    ...
    kfunc_id =
        find_ksym_btf_id(obj, ext->essent_name ?: ext->name, BTF_KIND_FUNC, &kern_btf, &mod_btf);
    if (kfunc_id < 0) {
    ...
    if (mod_btf && !mod_btf->fd_array_idx) {
        ...
        if (obj->fd_array_cnt == INT16_MAX) {
            pr_warn("extern (func ksym) '%s': module BTF fd index %d too big to fit in bpf_insn "
                    "offset\n",
                    ext->name,
        ...
        ret = libbpf_ensure_mem(
            (void **) &obj->fd_array, &obj->fd_array_cap, sizeof(int), obj->fd_array_cnt + 1);
        if (ret)
            return ret;
...
struct bpf_program *bpf_object__prev_program(const struct bpf_object *obj, struct bpf_program *next)
{
...
static int attach_kprobe_session(const struct bpf_program *prog,
                                 long cookie,
...
static const struct bpf_sec_def section_defs[] = {
    ...
    SEC_DEF(
        "sk_reuseport/migrate", SK_REUSEPORT, BPF_SK_REUSEPORT_SELECT_OR_MIGRATE, SEC_ATTACHABLE),
    ...
    SEC_DEF(
        "uretprobe.multi.s+", KPROBE, BPF_TRACE_UPROBE_MULTI, SEC_SLEEPABLE, attach_uprobe_multi),
    ...
    SEC_DEF(
        "fmod_ret.s+", TRACING, BPF_MODIFY_RETURN, SEC_ATTACH_BTF | SEC_SLEEPABLE, attach_trace),
    ...
    SEC_DEF(
        "cgroup/getpeername_unix", CGROUP_SOCK_ADDR, BPF_CGROUP_UNIX_GETPEERNAME, SEC_ATTACHABLE),
    SEC_DEF("cgroup/getsockname4", CGROUP_SOCK_ADDR, BPF_CGROUP_INET4_GETSOCKNAME, SEC_ATTACHABLE),
    SEC_DEF("cgroup/getsockname6", CGROUP_SOCK_ADDR, BPF_CGROUP_INET6_GETSOCKNAME, SEC_ATTACHABLE),
    SEC_DEF(
        "cgroup/getsockname_unix", CGROUP_SOCK_ADDR, BPF_CGROUP_UNIX_GETSOCKNAME, SEC_ATTACHABLE),
                                            ...
                                            Elf_Data *data)
{
    ...
    for (i = 0; i < nrels; i++) {
        ...
        st_ops = map->st_ops;
        pr_debug("struct_ops reloc %s: for %lld value %lld shdr_idx %u rel->r_offset %zu "
        ...
        if (shdr_idx >= SHN_LORESERVE) {
            pr_warn("struct_ops reloc %s: rel->r_offset %zu shdr_idx %u unsupported non-static "
                    "function\n",
                    map->name,
                    (size_t) rel->r_offset,
                    shdr_idx);
...
static inline int find_attach_btf_id(struct btf *btf,
                                     const char *name,
...
static int find_kernel_btf_id(struct bpf_object *obj,
                              const char *attach_name,
                              enum bpf_attach_type attach_type,
                              int *btf_obj_fd,
                              int *btf_type_id)
{
    ...
    if (!mod_name || strncmp(mod_name, "vmlinux", mod_len) == 0) {
        ret = find_attach_btf_id(obj->btf_vmlinux, mod_name ? fn_name : attach_name, attach_type);
        if (ret > 0) {
    ...
    for (i = 0; i < obj->btf_module_cnt; i++) {
        ...
        ret = find_attach_btf_id(mod->btf, mod_name ? fn_name : attach_name, attach_type);
        if (ret > 0) {
            *btf_obj_fd = mod->fd;
            *btf_type_id = ret;
            return 0;
...
static int map_btf_datasec_resize(struct bpf_map *map, __u32 size)
{
    ...
    if (element_sz <= 0 || (size - var->offset) % element_sz != 0) {
        pr_warn("map '%s': cannot be resized, element size (%d) doesn't align with new total size "
                "(%u)\n",
                bpf_map__name(map),
                element_sz,
                size);
...
static struct bpf_map *__bpf_map__iter(const struct bpf_map *m, const struct bpf_object *obj, int i)
{
    ssize_t idx;
    struct bpf_map *s, *e;
...
static int validate_map_op(const struct bpf_map *map,
                           size_t key_sz,
                           size_t value_sz,
                           bool check_value_sz)
{
    ...
    switch (map->def.type) {
        ...
        case BPF_MAP_TYPE_PERCPU_CGROUP_STORAGE: {
            ...
            if (value_sz != num_cpu * elem_sz) {
                pr_warn("map '%s': unexpected value size %zu provided for per-CPU map, expected %d "
                        "* %zu = %zd\n",
                        map->name,
                        value_sz,
                        num_cpu,
...
struct bpf_link *bpf_program__attach_perf_event_opts(const struct bpf_program *prog,
                                                     int pfd,
                                                     const struct bpf_perf_event_opts *opts)
{
    ...
    if (kernel_supports(prog->obj, FEAT_PERF_LINK) && !force_ioctl_attach) {
        DECLARE_LIBBPF_OPTS(bpf_link_create_opts,
                            link_opts,
...
static int perf_event_open_probe(
    bool uprobe, bool retprobe, const char *name, uint64_t offset, int pid, size_t ref_ctr_off)
{
...
static void gen_kprobe_legacy_event_name(char *buf,
                                         size_t buf_sz,
...
static int add_kprobe_event_legacy(const char *probe_name,
                                   bool retprobe,
                                   const char *kfunc_name,
                                   size_t offset)
{
    return append_to_file(tracefs_kprobe_events(),
                          "%c:%s/%s %s+0x%zx",
                          retprobe ? 'r' : 'p',
...
static int remove_kprobe_event_legacy(const char *probe_name, bool retprobe)
{
    return append_to_file(
        tracefs_kprobe_events(), "-:%s/%s", retprobe ? "kretprobes" : "kprobes", probe_name);
}
...
static int perf_event_kprobe_open_legacy(
    const char *probe_name, bool retprobe, const char *kfunc_name, size_t offset, int pid)
{
    const size_t attr_sz = sizeof(struct perf_event_attr);
    struct perf_event_attr attr;
    char errmsg[STRERR_BUFSIZE];
                                                 ...
                                                 const struct bpf_kprobe_opts *opts)
{
    ...
    if (!legacy) {
        pfd = perf_event_open_probe(
            false /* uprobe */, retprobe, func_name, offset, -1 /* pid */, 0 /* ref_ctr_off */);
    } else {
        char probe_name[256];
...
static int avail_kallsyms_cb(unsigned long long sym_addr,
                             char sym_type,
                             const char *sym_name,
                             void *ctx)
{
...
static int libbpf_available_kprobes_parse(struct kprobe_multi_resolve *res)
{
    ...
    while (true) {
        ...
        err =
            libbpf_ensure_mem((void **) &res->addrs, &res->cap, sizeof(*res->addrs), res->cnt + 1);
        if (err)
            goto cleanup;
...
static void gen_uprobe_legacy_event_name(char *buf,
                                         size_t buf_sz,
...
static inline int add_uprobe_event_legacy(const char *probe_name,
                                          bool retprobe,
                                          const char *binary_path,
                                          size_t offset)
{
    return append_to_file(tracefs_uprobe_events(),
                          "%c:%s/%s %s:0x%zx",
                          retprobe ? 'r' : 'p',
...
{
    return append_to_file(
        tracefs_uprobe_events(), "-:%s/%s", retprobe ? "uretprobes" : "uprobes", probe_name);
}
...
static int perf_event_uprobe_open_legacy(
    const char *probe_name, bool retprobe, const char *binary_path, size_t offset, int pid)
{
    const size_t attr_sz = sizeof(struct perf_event_attr);
    struct perf_event_attr attr;
    int type, pfd, err;
                                              ...
                                              const char *func_name)
{
    ...
    if (entry.compression) {
        pr_warn(
            "zip: entry %s of %s is compressed and cannot be handled\n", file_name, archive_path);
...
struct bpf_link *bpf_program__attach_uprobe_multi(const struct bpf_program *prog,
                                                  pid_t pid,
                                                  const char *path,
                                                            ...
                                                            const struct bpf_uprobe_opts *opts)
{
    ...
    if (!legacy) {
        pfd = perf_event_open_probe(
            true /* uprobe */, retprobe, binary_path, func_offset, pid, ref_ctr_off);
    } else {
                                          ...
                                          const struct bpf_usdt_opts *opts)
{
    ...
    link = usdt_manager_attach_usdt(
        obj->usdt_man, prog, pid, binary_path, usdt_provider, usdt_name, usdt_cookie);
    err = libbpf_get_error(link);
    if (err)
        return libbpf_err_ptr(err);
    return link;
...
struct bpf_link *bpf_program__attach_raw_tracepoint_opts(const struct bpf_program *prog,
                                                         const char *tp_name,
                                              ...
                                              const char *attach_func_name)
{
    ...
    if (target_fd) {
        ...
        return bpf_program_attach_fd(prog, target_fd, "freplace", &target_opts);
    } else {
                                                      ...
                                                      int map_key)
{
    ...
    cpu_buf->base = mmap(
        NULL, pb->mmap_size + pb->page_size, PROT_READ | PROT_WRITE, MAP_SHARED, cpu_buf->fd, 0);
...
static struct perf_buffer *__perf_buffer__new(int map_fd,
                                              size_t page_cnt,
...
struct perf_buffer *perf_buffer__new_raw(int map_fd,
                                         size_t page_cnt,
                                         struct perf_event_attr *attr,
                                         perf_buffer_event_fn event_cb,
                                         void *ctx,
...
static enum bpf_perf_event_ret perf_buffer__process_record(struct perf_event_header *e, void *ctx)
{
    ...
    switch (e->type) {
        ...
        case PERF_RECORD_LOST: {
            ...
            printf("Received lost event: %d\n", s->lost);

            if (pb->lost_cb) {
                printf("Calling lost callback\n");
                pb->lost_cb(pb->ctx, cpu_buf->cpu, s->lost);
            } else {
                printf("Lost callback is nil\n");
            }
            break;
        }
        default:
...
static int perf_buffer__process_records(struct perf_buffer *pb, struct perf_cpu_buf *cpu_buf)
{
    ...
    ret = perf_event_read_simple(cpu_buf->base,
                                 pb->mmap_size,
                                 pb->page_size,
                                 &cpu_buf->buf,
                                 &cpu_buf->buf_size,
...
int perf_buffer__poll(struct perf_buffer *pb, int timeout_ms)
{
    ...
    if (cnt < 0) {
        printf("libbpf: error polling perf buffer: %d\n", errno);
        fprintf(stderr, "libbpf: error polling perf buffer: %d\n", errno);
        return -errno;
    }
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (err) {
            printf("libbpf: error while processing records: %d\n", err);
            fprintf(stderr, "libbpf: error while processing records: %d\n", err);
                                   ...
                                   const char *attach_func_name)
{
    ...
    if (attach_prog_fd) {
        ...
        if (err)
            return libbpf_err(err);
        err = find_kernel_btf_id(
            prog->obj, attach_func_name, prog->expected_attach_type, &btf_obj_fd, &btf_id);
        if (err)
            return libbpf_err(err);
    }
